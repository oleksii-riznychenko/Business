...
function adaptive_header(w, h) {
	...
	var headermenus = $('.header-menu__items');
	if (w < 991.98) {
		if (!headermenus.hasClass('done')) {
			headermenus.addClass('done').appendTo(headerMenu);
		}
	} else {
		if (headermenus.hasClass('done')) {
			headermenus.removeClass('done').prependTo($('.header__column--right'));
		}
	}
}
